...
class Config:
    ...
    @staticmethod
    def validate_feature_flags(config_dict: Dict, prefix: str = "") -> List[str]:
        errors = []
            ...
            if key.startswith(FEATURE_FLAG_PREFIX):
                if not isinstance(value, bool):
                    if isinstance(value, str) and value.lower() in ("true", "false"):
                        continue
                    errors.append(f"Feature flag '{full_key}' has non-boolean value: {value}")
            if isinstance(value, dict):
                errors.extend(Config.validate_feature_flags(value, full_key))
        return errors

    def validate(self) -> List[str]:
        return self.validate_feature_flags(self._data)
