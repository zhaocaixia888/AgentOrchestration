...
from src.common.cascade_deletion import cascade_deletion_manager
...
class OrchestrationEngine:
    """Orchestration engine with token revalidation for long polling.

    Fixes #625: Every long-poll loop iteration revalidates the caller's token
    against the live revocation set. Stale/revoked tokens force an early exit.
    """
    ...
    def __init__(self, max_workers: int = 10, agent_timeout: int = 300):
        ...
        # Token tracking for long-poll revalidation (#625)
        self._poll_tokens: Dict[str, Dict] = {}
    ...
    def register_poll_token(self, poll_id: str, raw_token: str) -> None:
        """Register a token for long-poll revalidation (#625)."""
        self._poll_tokens[poll_id] = {
            "token": raw_token,
            "last_validated_at": time.time(),
        }

    def unregister_poll_token(self, poll_id: str) -> None:
        """Unregister a token when the poll connection closes."""
        self._poll_tokens.pop(poll_id, None)

    def _revalidate_poll_token(self, poll_id: str) -> bool:
        """Revalidate a long-poll token. Returns False if token is revoked/stale.

        Fixes #625: Called on every poll iteration to ensure the caller's
        credentials are still valid. If revoked, the poll is terminated.
        """
        poll_info = self._poll_tokens.get(poll_id)
        if poll_info is None:
            return False

        raw_token = poll_info["token"]
        last_validated = poll_info["last_validated_at"]

        if auth_service.revalidate_for_long_poll(raw_token, last_validated):
            poll_info["last_validated_at"] = time.time()
            return True

        logger.warning(f"Long-poll token revalidation failed for poll_id={poll_id}")
        return False

    async def start(self, poll_id: Optional[str] = None) -> None:
        self._running = True
        logger.info("Orchestration engine started")
        while self._running:
            # Revalidate token on every poll iteration (#625)
            if poll_id and not self._revalidate_poll_token(poll_id):
                logger.warning(f"Stopping engine for poll_id={poll_id}: token revoked")
                self._running = False
                break
    ...
    def cancel_parent(self, parent_id: str) -> None:
        ...
        self._parent_reducer.transition(parent_id, EngineState.FAILED)
        logger.info("Parent %s cancelled", parent_id)

    def is_parent_cancelled(self, parent_id: str) -> bool:
        return parent_id in self._cancelled_parents
    ...
    async def _execute_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if parent_id:
                if self.is_parent_cancelled(parent_id):
                    logger.warning("Parent %s cancelled, skipping retry for task %s", parent_id, task_id)
                    return
            ...
            if parent_id:
                if self.is_parent_cancelled(parent_id):
                    logger.warning("Parent %s cancelled, suppressing child retry for task %s", parent_id, task_id)
                    return
                self._parent_reducer.transition(parent_id, EngineState.FAILED, attempt=parent_attempt, revision=parent_revision)
