...
class OrchestrationEngine:
    ...
    async def _execute_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if parent_id:
                if parent_id in self._cancelled_parents or self._parent_reducer.get_state(parent_id) == EngineState.FAILED:
                    raise ValueError("Parent %s has failed, cannot start subworkflow" % parent_id)
                if parent_id not in self._child_events:
                    self.initialize_parent(parent_id, parent_attempt, parent_revision)
                    self._parent_reducer.transition(parent_id, EngineState.RUNNING, attempt=parent_attempt, revision=parent_revision)
